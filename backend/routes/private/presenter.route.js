...
presenterRouter.get(
  ...
  async (req, res) => {
    ...
    if (!req.user) {
      ...
      return res.redirect(
        `${process.env.CLIENT_BASE_URL}/auth?error=Authentication%20failed.`
      );
    }
    ...
    try {
      ...
      res.redirect(
        `${process.env.CLIENT_BASE_URL}/auth/success?token=${encodeURIComponent(
          token
      ...
      res.redirect(
        `${process.env.CLIENT_BASE_URL}/auth?error=Something%20went%20wrong.`
      );
    }
  }
);
