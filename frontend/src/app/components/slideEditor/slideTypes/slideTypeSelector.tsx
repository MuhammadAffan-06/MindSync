  ...
  return (
    <div className="grid grid-cols-3 grid-rows-2 m-auto h-2/3 rounded-3xl w-1/2 text-center items-center bg-[var(--background)] align-middle justify-center">
      {buttonArray.map((item, index) => (
        <div
          key={index}
          onClick={() => !item.disabled && setSlideType(id, item.type)}
          className={`flex flex-col items-center justify-center cursor-pointer
            ${item.disabled ? "opacity-50 cursor-not-allowed" : "hover:translate-y-[-1px]"}`}
        >
          <Image
            src={item.icon}
            width={40}
            ...
            title={item.label}
            alt={item.label}
            className="w-24 h-24 p-6 rounded-xl bg-gradient-to-b from-[var(--secondary-color)] to-[#4830E2]"
          />
